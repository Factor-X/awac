...
(
  ...
  code character varying(255),
  retricted_scope character varying(255),
...
(
  ...
  CONSTRAINT fk_mm_report_indicator_indicator_id FOREIGN KEY (indicator_id)
      ...
      ON UPDATE NO ACTION ON DELETE CASCADE,
  CONSTRAINT uk_mm_report_indicator_pkey_logical_pkey UNIQUE (indicator_id, report_id)
)
WITH (
  OIDS=FALSE
);
...
(
  ...
  CONSTRAINT fk_baseindicator_unit_id FOREIGN KEY (unit_id)
      REFERENCES unit (id) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT uk_baseindicator_logical_pkey UNIQUE (code)
)
WITH (
  OIDS=FALSE
);
...
(
  ...
  CONSTRAINT fk_mm_indicator_baseindicator_indicator_id FOREIGN KEY (indicator_id)
      REFERENCES indicator (id) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE CASCADE,
  CONSTRAINT fk_mm_indicator_baseindicator_baseindicator_id FOREIGN KEY (baseindicator_id)
      REFERENCES baseindicator (id) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE CASCADE,
  CONSTRAINT uk_mm_indicator_baseindicator_logical_pkey UNIQUE (indicator_id, baseindicator_id)
)
WITH (
  OIDS=FALSE
);
