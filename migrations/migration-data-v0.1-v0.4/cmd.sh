...
pg_dump --data-only --column-inserts -h ec2-54-217-208-158.eu-west-1.compute.amazonaws.com -d d2onsan1iugu6d -U pmouzzkdtqbmmi -w | egrep -o '^INSERT INTO questionsetanswer.*$|^INSERT INTO question_answer.*$|^INSERT INTO answer_value.*$|^SELECT pg_catalog.setval.*$|^INSERT INTO booleananswervalue.*$|^INSERT INTO codeanswervalue.*$|^INSERT INTO doubleanswervalue.*$|^INSERT INTO integeranswervalue.*$|^INSERT INTO stringanswervalue.*$|^INSERT INTO formprogress.*$' > /tmp/migration/useful_data.sql
...
# clean up empty question_answers
echo "delete from question_answer WHERE id in ( select id from (select qa.id, COUNT(av.id) as cnt from question_answer qa LEFT OUTER JOIN answer_value av on av.questionanswer_id = qa.id GROUP BY qa.id HAVING COUNT(av.id) = 0) t );" >> /tmp/migration/useful_data.sql
