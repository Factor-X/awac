...
object CdnWebJarAssets extends WebJarAssets(Assets) {
  ...
  def getUrl(file: String) = {
    ...
    maybeContentUrl.map { contentUrl =>
        contentUrl + eu.factorx.awac.controllers.routes.CdnWebJarAssets.at(file).url
    } getOrElse eu.factorx.awac.controllers.routes.CdnWebJarAssets.at(file).url
  }
...
object RequireJS {
  ...
  def setup(main: String): String = {
    ...
    def nastyReflectedRoute(path: String, routerName: String, packPath: String): Call = {
      // We have to use the current Play app's classloader otherwise we won't be able to find the reverse routers
      val c = Class.forName(packPath + "controllers.Reverse" + routerName, true, play.api.Play.current.classloader)
    ...
    def nastyReflectedWebJarAssetsRoute(path: String): Call = nastyReflectedRoute(path, "CdnWebJarAssets","eu.factorx.awac.")
