...
public class AwacDummyDataCreator {
	...
	public static void createAwacDummyData(ApplicationContext ctx, Session session) {
		 ...
		 * // ANSWERS
		 * 
		 * // -- FORM Form tab1Form = new Form("TAB1"); session.saveOrUpdate(tab1Form);
		 * 
		 * Form tab2Form = new Form("TAB2"); session.saveOrUpdate(tab2Form);
		 * 
		 * Form tab4Form = new Form("TAB4"); session.saveOrUpdate(tab4Form);
		 * 
		 * 
		 * // -- QUESTION_SETS QuestionSet a1 = new QuestionSet(QuestionCode.A1, false); session.saveOrUpdate(a1); tab1Form.getQuestionSets().add(a1);
		 * session.saveOrUpdate(tab1Form);
		 * 
		 * 
		 * QuestionSet a13 = new QuestionSet(QuestionCode.A13, false); session.saveOrUpdate(a13); tab2Form.getQuestionSets().add(a13); session.saveOrUpdate(tab2Form);
		 * 
		 * QuestionSet a15 = new QuestionSet(QuestionCode.A15, true); session.saveOrUpdate(a15); tab2Form.getQuestionSets().add(a15); session.saveOrUpdate(tab2Form);
		 * 
		 * QuestionSet a20 = new QuestionSet(QuestionCode.A20, false); session.saveOrUpdate(a20); tab2Form.getQuestionSets().add(a20); session.saveOrUpdate(tab2Form);
		 * 
		 * QuestionSet a22 = new QuestionSet(QuestionCode.A22, false); session.saveOrUpdate(a22); tab2Form.getQuestionSets().add(a22); session.saveOrUpdate(tab2Form);
		 * 
		 * QuestionSet a25 = new QuestionSet(QuestionCode.A25, false); session.saveOrUpdate(a25); tab2Form.getQuestionSets().add(a25); session.saveOrUpdate(tab2Form);
		 * 
		 * QuestionSet a31 = new QuestionSet(QuestionCode.A31, false); session.saveOrUpdate(a31); tab2Form.getQuestionSets().add(a31); session.saveOrUpdate(tab2Form);
		 * 
		 * QuestionSet a41 = new QuestionSet(QuestionCode.A41, false); session.saveOrUpdate(a41); tab2Form.getQuestionSets().add(a41); session.saveOrUpdate(tab2Form);
		 * 
		 * QuestionSet a42 = new QuestionSet(QuestionCode.A42, false); session.saveOrUpdate(a42); tab2Form.getQuestionSets().add(a42); session.saveOrUpdate(tab2Form);
		 * 
		 * QuestionSet a45 = new QuestionSet(QuestionCode.A45, false); session.saveOrUpdate(a45); tab2Form.getQuestionSets().add(a45); session.saveOrUpdate(tab2Form);
		 * 
		 * QuestionSet a47 = new QuestionSet(QuestionCode.A47, false); session.saveOrUpdate(a47); tab2Form.getQuestionSets().add(a47); session.saveOrUpdate(tab2Form);
		 * 
		 * 
		 * // -- QUESTIONS
		 * 
		 * session.saveOrUpdate(new IntegerQuestion(a1, 0, QuestionCode.A2, null)); session.saveOrUpdate(new ValueSelectionQuestion(a1, 100, QuestionCode.A3,
		 * CodeList.SITE_SECTORS)); session.saveOrUpdate(new ValueSelectionQuestion(a1, 200, QuestionCode.A4, CodeList.NACE_CODES_1)); session.saveOrUpdate(new
		 * ValueSelectionQuestion(a1, 300, QuestionCode.A5, CodeList.NACE_CODES_2)); session.saveOrUpdate(new ValueSelectionQuestion(a1, 400, QuestionCode.A6,
		 * CodeList.NACE_CODES_3)); session.saveOrUpdate(new BooleanQuestion(a1, 500, QuestionCode.A7)); session.saveOrUpdate(new ValueSelectionQuestion(a1, 600, QuestionCode.A8,
		 * CodeList.PUBLIC_PRIVATE)); session.saveOrUpdate(new DoubleQuestion(a1, 700, QuestionCode.A9, surfaceUnits)); session.saveOrUpdate(new DoubleQuestion(a1, 800,
		 * QuestionCode.A10, surfaceUnits)); session.saveOrUpdate(new BooleanQuestion(a1, 900, QuestionCode.A11)); session.saveOrUpdate(new IntegerQuestion(a1, 1000,
		 * QuestionCode.A12, null));
		 * 
		 * // TAB 2
		 * 
		 * session.saveOrUpdate(new IntegerQuestion(a13, 0, QuestionCode.A14, null)); session.saveOrUpdate(new IntegerQuestion(a13, 100, QuestionCode.A15, null)); Question a16 =
		 * new ValueSelectionQuestion(a15, 200, QuestionCode.A16, CodeList.ACTIVITY_SOURCE); session.saveOrUpdate(a16); Question a17 = new DoubleQuestion(a15, 300,
		 * QuestionCode.A17, energyUnits); session.saveOrUpdate(a17); // 18 // 19 session.saveOrUpdate(new IntegerQuestion(a13, 400, QuestionCode.A21, null));
		 * session.saveOrUpdate(new IntegerQuestion(a13, 500, QuestionCode.A23, null)); session.saveOrUpdate(new IntegerQuestion(a13, 600, QuestionCode.A24, null));
		 * 
		 * 
		 * // First set of answers int repetitionIndex = 0; QuestionSetAnswer questionSet15Answer0 = new QuestionSetAnswer(scope, period1, a15, repetitionIndex, null);
		 * session.saveOrUpdate(questionSet15Answer0);
		 * 
		 * QuestionAnswer a16Answer0 = new QuestionAnswer(user1, null, questionSet15Answer0, a16); session.saveOrUpdate(a16Answer0); CodeAnswerValue a16Answer0Value = new
		 * CodeAnswerValue(a16Answer0, ActivitySourceCode.DIESEL_GASOIL_OU_FUEL_LEGER); session.saveOrUpdate(a16Answer0Value);
		 * 
		 * QuestionAnswer a17Answer0 = new QuestionAnswer(user1, null, questionSet15Answer0, a17); session.saveOrUpdate(a17Answer0); DoubleAnswerValue a17Answer0Value = new
		 * DoubleAnswerValue(a17Answer0, 20.6, gj); session.saveOrUpdate(a17Answer0Value);
		 * 
		 * // Second set of answers repetitionIndex = 1; QuestionSetAnswer questionSet15Answer1 = new QuestionSetAnswer(scope, period1, a15, repetitionIndex, null);
		 * session.saveOrUpdate(questionSet15Answer1);
		 * 
		 * QuestionAnswer a16Answer1 = new QuestionAnswer(user1, null, questionSet15Answer1, a16); session.saveOrUpdate(a16Answer1); CodeAnswerValue a16Answer1Value = new
		 * CodeAnswerValue(a16Answer1, ActivitySourceCode.BOIS_BUCHE); session.saveOrUpdate(a16Answer1Value);
		 * 
		 * QuestionAnswer a17Answer1 = new QuestionAnswer(user1, null, questionSet15Answer1, a17); session.saveOrUpdate(a17Answer1); DoubleAnswerValue a17Answer1Value = new
		 * DoubleAnswerValue(a17Answer1, 60523.0, kcal); session.saveOrUpdate(a17Answer1Value);
		 * 
		 * 
		 ...
		 * // -- Question sets
		 * 
		 * // Question Set "entrepôts de stockage" QuestionSet a164 = new QuestionSet(QuestionCode.A164, true, null); session.saveOrUpdate(a164);
		 * tab4Form.getQuestionSets().add(a164); session.saveOrUpdate(tab4Form);
		 * 
		 * // -- Question "nom" Question a165 = new StringQuestion(a164, 0, QuestionCode.A165); session.saveOrUpdate(a165);
		 * 
		 * // -- Question Set "combustibles" QuestionSet a166 = new QuestionSet(QuestionCode.A166, true, a164); session.saveOrUpdate(a166); // -- -- Question "type" Question a167 =
		 * new ValueSelectionQuestion(a166, 0, QuestionCode.A167, CodeList.FUEL); session.saveOrUpdate(a167); // -- -- Question "quantité" Question a168 = new DoubleQuestion(a166,
		 * 1, QuestionCode.A168, energyUnits); session.saveOrUpdate(a168);
		 * 
		 * // -- Question "électricité" Question a169 = new DoubleQuestion(a164, 1, QuestionCode.A169, energyUnits); session.saveOrUpdate(a169);
		 * 
		 * // -- Question Set "gaz réfrigérants" QuestionSet a170 = new QuestionSet(QuestionCode.A170, true, a164); session.saveOrUpdate(a170); // -- -- Question "type" Question
		 * a171 = new ValueSelectionQuestion(a170, 0, QuestionCode.A171, CodeList.REFRIGERANT_GAS); session.saveOrUpdate(a171); // -- -- Question "quantité" Question a172 = new
		 * DoubleQuestion(a170, 1, QuestionCode.A172, massUnits); session.saveOrUpdate(a172);
		 * 
		 * // -- Answers
		 * 
		 * // 1er entrepôt QuestionSetAnswer qs164_r0 = new QuestionSetAnswer(scope, period1, a164, 0, null);
		 * 
		 * // -- -- 1er combustible QuestionSetAnswer qs164_r0_qs166_r0 = new QuestionSetAnswer(scope, period1, a166, 0, qs164_r0); // -- -- -- type // -- -- -- quantité // -- --
		 * -- électricité
		 * 
		 * // -- -- 2ème combustible QuestionSetAnswer qs164_r0_qs166_r1 = new QuestionSetAnswer(scope, period1, a166, 1, qs164_r0); // -- -- -- type // -- -- -- quantité // -- --
		 * -- électricité
		 * 
		 * // -- -- 1er gaz réfrigérant QuestionSetAnswer qs164_r0_qs170_r0 = new QuestionSetAnswer(scope, period1, a170, 0, qs164_r0); // -- -- -- type // -- -- -- quantité
		 * 
		 * // 2ème entrepôt QuestionSetAnswer qs164_r1 = new QuestionSetAnswer(scope, period1, a164, 1, null);
		 * 
		 * // -- -- 1er combustible QuestionSetAnswer qs164_r1_qs166_r0 = new QuestionSetAnswer(scope, period1, a166, 0, qs164_r1); // -- -- -- type // -- -- -- quantité // -- --
		 * -- électricité
		 * 
		 * // -- -- 1er gaz réfrigérant QuestionSetAnswer qs164_r1_qs170_r0 = new QuestionSetAnswer(scope, period1, a170, 0, qs164_r1); // -- -- -- type // -- -- -- quantité
		 * 
		 * // -- -- 2ème gaz réfrigérant QuestionSetAnswer qs164_r1_qs170_r1 = new QuestionSetAnswer(scope, period1, a170, 1, qs164_r1); // -- -- -- type // -- -- -- quantité
		 */
	}
