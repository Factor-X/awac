...
public class ReportResultServiceImpl implements ReportResultService {
    ...
    @Override
    public Map<String, List<Double>> getScopeValuesByIndicator(ReportResult reportResult) {
        ...
        for (Indicator indicator : reportResult.getReport().getIndicators()) {
            ...
            if (reportResult.getActivityResults().containsKey(indicatorCode)) {
                indicatorActivityResults = reportResult.getActivityResults().get(indicatorCode);
            } else {
        ...
        for (Map.Entry<String, List<BaseActivityResult>> indicatorData : dataByIndicator.entrySet()) {
            ...
            for (BaseActivityResult baseActivityResult : indicatorData.getValue()) {
                Double numericValue = baseActivityResultService.getValueForYear(baseActivityResult);
        ...
        for (BaseIndicator baseIndicator : baseIndicators) {
            ...
            for (BaseActivityData baseActivityData : indicatorBADs) {
                ...
                if (factor == null) {
                    ...
                    BaseActivityResult bar = new BaseActivityResult(baseIndicator, baseActivityData, factor, site, Integer.valueOf(period.getPeriodCode().getKey()));
