...
@Component
public class ReportServiceWithRankImpl implements ReportService {
	...
	public Report getReport(Scope scope, Period period) {
		...
		Set<BaseActivityDataCode> usedActivityData = new HashSet<>();
		...
		for (Indicator indicator : indicators) {
			...
			for (BaseActivityData baseActivityData : activityData) {
				...
				if (factor == null) {
					...
					usedActivityData.add(baseActivityData.getKey());
				}
			}
		}
		...
		for (BaseActivityData bad : allActivityData) {
			if (!usedActivityData.contains(bad.getKey())) {
...
//			}
			...
			if (minRankByAlternativeGroup.containsKey(alternativeGroup)) {
				Integer minRank = minRankByAlternativeGroup.get(alternativeGroup);
				if (rank < minRank) {
					minRankByAlternativeGroup.put(alternativeGroup, rank);
				}
			} else {
				minRankByAlternativeGroup.put(alternativeGroup, rank);
			}
