...
angular.module('app.directives').directive('mmNullValidator', function(){
  ...
  return {
    ...
    controller: function($scope, modalService) {
      ...
      $scope.$watch('fields.percentOwned.field', function() {
        console.log('-->' + parseFloat($scope.fields.percentOwned.field));
        if (parseFloat($scope.fields.percentOwned.field) < 100) {
          $scope.fields.orgStructure.hidden = false;
          $scope.fields.ecoInterest.hidden = false;
          $scope.fields.opePolicy.hidden = false;
          return $scope.fields.accountingTreatment.hidden = false;
        } else {
          $scope.fields.orgStructure.hidden = true;
          $scope.fields.ecoInterest.hidden = true;
          $scope.fields.opePolicy.hidden = true;
          return $scope.fields.accountingTreatment.hidden = true;
        }
      });
      $scope.fields = {
        name: {
