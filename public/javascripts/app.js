...
});angular.module('app.directives').directive("mmAwacSection", function(directiveService, downloadService, messageFlash, modalService, $location) {
  return {
    restrict: "E",
    scope: directiveService.autoScope({
      ngTitleCode: '=',
    ...
    link: function(scope) {
      ...
      scope.valide = function() {
        var data;
        if ((!scope.isValidate() || scope.isValidateByMyself() || scope.$root.currentPerson.isAdmin) && scope.$root.closedForms === false && (scope.$root.verificationRequest === null || (scope.$root.verificationRequest.status === 'VERIFICATION_STATUS_CORRECTION' && scope.$parent.getQuestionSetVerification(scope.getTitleCode()).status === 'rejected'))) {
          data = {};
          data.questionSetKey = scope.getTitleCode();
          data.periodCode = scope.$root.periodSelectedKey;
          data.scopeId = scope.$root.scopeSelectedId;
      ...
      scope.getValidateClass = function() {
        if (scope.isValidate()) {
          if ((scope.isValidateByMyself() || scope.$root.currentPerson.isAdmin) && scope.$root.closedForms === false && (scope.$root.verificationRequest === null || (scope.$root.verificationRequest.status === 'VERIFICATION_STATUS_CORRECTION' && scope.$parent.getQuestionSetVerification(scope.getTitleCode()).status === 'rejected'))) {
    ...
    controller: function($scope, modalService) {
      ...
      $scope.save = function() {
        if ($scope.allFieldValid()) {
          $scope.isLoading = true;
          downloadService.postJson($scope.getParams().url, $scope.getParams().data, function(result) {
...
});angular.module('app.controllers').controller("VerificationSubmitCtrl", function($scope, displayLittleFormMenu, downloadService, modalService, messageFlash, translationService, ngTableParams, $filter, $compile) {
  $scope.displayLittleFormMenu = displayLittleFormMenu;
  $scope.selectedRequest = null;
  $scope.form = null;
  $scope.requestSelected = null;
