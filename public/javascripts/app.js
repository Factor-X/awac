...
});angular.module('app.controllers').controller("SiteManagerCtrl", function($scope, translationService) {});angular.module('app.controllers').controller("UserDataCtrl", function($scope, downloadService, translationService, messageFlash, modalService, $timeout) {
  $scope.isLoading = false;
  $scope.identifierInfo = {
    fieldTitle: "USER_IDENTIFIER",
    disabled: true,
  ...
  $scope.activeUser = function(user) {
    ...
    return downloadService.postJson("/awac/user/activeAccount", data, function(result) {
      if (result.success) {
        ...
        return $scope.isLoading['isActive'][user] = false;
      } else {
        return $scope.isLoading['isActive'][user] = false;
      }
    });
