...
angular.module('app.directives').directive('mmNotNullValidator', function(){
      ...
      if ($scope.getParams().event != null) {
        ...
        if ($scope.$root.periods != null) {
          ...
          $scope.$watch('$root.periods', function() {
            return $scope.assignedPeriod = angular.copy($scope.$root.periods[0].label);
          });
        }
      }
      $scope.fields = {
  ...
  return {
    ...
    link: function(scope, element) {
      ...
      return scope.getTotalOutOfScope = function() {
        var rl, total, _i, _len, _ref;
        if (!scope.ngModel) {
          return void 0;
        }
  ...
  */
  ...
  $scope.$watch('$root.periodToCompare', function() {
    if ($scope.$parent !== null && ($scope.$root.periodToCompare != null)) {
...
angular.module('app.controllers').controller("MainCtrl", function($scope, downloadService, translationService, $sce, $location, $route, $routeParams, modalService, $timeout, messageFlash) {
  ...
  $scope.nav = function(loc, confirmed) {
    ...
    if (canBeContinue) {
      return $location.path(loc + "/" + $scope.$root.periodSelectedKey + "/" + $scope.$root.scopeSelectedId);
    }
  };
  ...
  $scope.$watchCollection('[$root.periodSelectedKey,$root.scopeSelectedId]', function() {
    var k, p, url, v;
  ...
  $scope.loadPeriodForComparison = function() {
    var url;
    if (!$scope.$root.scopeSelectedId && !isNaN($scope.$root.scopeSelectedId)) {
      ...
      return downloadService.getJson(url, function(result) {
        ...
        if (result.success) {
          ...
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            period = _ref[_i];
            _results.push(period.key !== $scope.$root.periodSelectedKey ? $scope.periodsForComparison[$scope.periodsForComparison.length] = period : void 0);
          }
          return _results;
        } else {
          return messageFlash.displayError(result.data.message);
...
angular.module('app').run(function($rootScope, $location, downloadService, messageFlash, $timeout, translationService, tmhDynamicLocale, $routeParams) {
  ...
  $rootScope.testForm = function(period, scope) {
    var periodToFind, site, _i, _j, _len, _len2, _ref, _ref2;
  ...
  $rootScope.loginSuccess = function(data, skipRedirect) {
    var period, site, _i, _j, _k, _len, _len2, _len3, _ref, _ref2, _ref3;
    ...
    if (data.defaultSiteId != null) {
      _ref = $rootScope.mySites;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        site = _ref[_i];
        if (site.id === data.defaultSiteId) {
          $rootScope.scopeSelectedId = data.defaultSiteId;
          if (data.defaultPeriod != null) {
            if (typeof site.listPeriodAvailable === "function" ? site.listPeriodAvailable(site.listPeriodAvailable.length > 0) : void 0) {
              _ref2 = site.listPeriodAvailable;
              for (_j = 0, _len2 = _ref2.length; _j < _len2; _j++) {
                period = _ref2[_j];
  ...
  $rootScope.computePeriod = function(scopeId) {
    var site, _i, _len, _ref, _results;
    _ref = $rootScope.mySites;
    _results = [];
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      site = _ref[_i];
      _results.push(site.scope === scopeId ? $rootScope.availablePeriods = site.listPeriodAvailable : void 0);
    }
    return _results;
  };
  ...
  return $rootScope.$on("$routeChangeSuccess", function(event, current, previous) {
    ...
    return console.log("change route : " + $location.path());
  });
  ...
  return downloadService.getJson('awac/organization/getMyOrganization', function(result) {
    ...
    if (!result.success) {
      ...
      downloadService.postJson('awac/organization/events/load', data, function(result) {
        ...
        console.log("RESULT");
        console.log(result);
        return console.log(result.data.organizationEventList);
      });
      $scope.$watch('assignPeriod', function() {
        var site, _i, _len, _ref, _results;
      ...
      $scope.assignPeriodToSite = function(site) {
        ...
        return downloadService.postJson('awac/site/assignPeriodToSite', data, function(result) {
          ...
          if (!result.success) {
            return messageFlash.displayError('Unable to load data...');
          } else {
            if (!(site.listPeriodAvailable != null)) {
            ...
            for (_i = 0, _len = _ref.length; _i < _len; _i++) {
              period = _ref[_i];
              _results.push(period.id === $scope.assignPeriod ? site.listPeriodAvailable[site.listPeriodAvailable.length] = period : void 0);
            }
            return _results;
          }
        });
...
});angular.module('app.controllers').controller("ResultsCtrl", function($scope, $window, downloadService, displayFormMenu, modalService, messageFlash) {
  $scope.displayFormMenu = displayFormMenu;
  $scope.$watch('$root.mySites|filter:{$selected:true}', function(nv) {
    return $scope.mySites = $scope.$root.mySites;
  }, true);
  $scope.$watch('mySites|filter:{$selected:true}', function(sites) {
    var dto;
    console.log("sites == ");
    console.log(sites);
    if (sites.length > 0) {
      ...
      return downloadService.postJson('/awac/result/getReport', dto, function(result) {
        ...
        if (result.success) {
          ...
          return messageFlash.displayError("Unable to load report");
        }
      });
    }
  }, true);
