...
angular.module('app').run(function($rootScope) {
  ...
  return;
});angular.module('app.services').service("translationService", function($rootScope, downloadService, $filter) {
  var svc;
  svc = this;
  svc.elements = null;
...
angular.module('app.controllers').controller("MainCtrl", function($scope, downloadService, translationService, $sce, $location, $route, $routeParams, modalService, $timeout, messageFlash, $compile, $element, $window) {
  ...
  $scope.test1 = function() {
    return downloadService.getJson("/test1", function(result) {
      if (!result.success) {
        return messageFlash.displayError(translationService.translateExceptionsDTO(result.data));
      }
    });
  };
  $scope.test2 = function() {
    return downloadService.getJson("/test2", function(result) {
      if (!result.success) {
        return messageFlash.displayError(translationService.translateExceptionsDTO(result.data));
      }
    });
