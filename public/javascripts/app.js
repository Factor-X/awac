...
angular.module('app').run(function($rootScope, $location, downloadService, messageFlash, $timeout, translationService, tmhDynamicLocale, $routeParams, $route, modalService) {
  ...
  */
...
});angular.module('app.controllers').controller("AdminFactorsManagerCtrl", function($scope, displayLittleFormMenu, downloadService, modalService, messageFlash, translationService, ngTableParams, $filter, $compile) {
  $scope.displayLittleFormMenu = displayLittleFormMenu;
  $scope.factors = null;
  modalService.show(modalService.LOADING);
  downloadService.getJson("/awac/admin/factors/all", function(result) {
    modalService.close(modalService.LOADING);
    if (result.success === true) {
      return $scope.factors = result.data.list;
    }
  });
  $scope.save = function() {
    modalService.show(modalService.LOADING);
  ...
  return $scope.remove = function(factor) {};
