...
angular.module('app.directives').directive('mmRangeValidator', function(){
  ...
  return {
    ...
    controller: function($scope, modalService) {
      var actionAdvice, indicatorCodeLabels, indicatorOptionsByInterfaceType, interfaceTypeCodeLabels, interfaceTypeKeys, _ref, _ref2;
      directiveService.autoScopeImpl($scope);
      ...
      indicatorOptionsByInterfaceType = {
        enterprise: _.filter(indicatorCodeLabels, function(codeLabel) {
          return codeLabel.key.startsWith("BI_");
        }),
        municipality: _.filter(indicatorCodeLabels, function(codeLabel) {
      ...
      $scope.indicatorAssociations = (_ref2 = $scope.actionAdvice.baseIndicatorAssociations) != null ? _ref2 : [];
      ...
      $scope.getIndicatorOptions = function() {
        var indicatorOptions, selectedIndicatorsKeys;
        ...
        if ($scope.indicatorAssociations.length > 0) {
          ...
          indicatorOptions = _.reject(indicatorOptions, function(codeLabel) {
            return _.contains(selectedIndicatorsKeys, codeLabel.key);
          });
        }
        indicatorOptions = _.sortBy(indicatorOptions, function(codeLabel) {
      ...
      $scope.allFieldValid = function() {
        return $scope.title.isValid && $scope.physicalMeasure.isValid && $scope.webSite.isValid && $scope.responsiblePerson.isValid && $scope.comment.isValid && $scope.indicatorAssociations.length > 0;
      };
      $scope.save = function() {
        var data;
        $scope.isLoading = true;
      ...
      $scope.addIndicatorAssociation = function() {
        if ($scope.indicatorAssociationToAdd.isValid()) {
          ...
          $scope.indicatorAssociations = _.sortBy($scope.indicatorAssociations, function(indicatorAssociation) {
      ...
      $scope.removeIndicatorAssociation = function(indicatorKey) {
        var biAssociation, index, _ref;
        _ref = $scope.indicatorAssociations;
        for (index in _ref) {
          biAssociation = _ref[index];
...
});angular.module('app.directives').directive("mmDatePicker", function(directiveService, $filter, generateId) {
  return {
    restrict: "E",
    scope: directiveService.autoScope({
      ngModel: '='
    }),
    templateUrl: "$/angular/templates/mm-date-picker.html",
    replace: true,
    transclude: true,
    compile: function() {
      return {
        ...
        post: function(scope) {
          ...
          return scope.$watch('ngModel', function() {
            return scope.resultFormated = $filter('date')(scope.ngModel, 'yyyy-MM-dd');
          });
        }
      };
    }
  };
});angular.module('app.directives').directive("mmTable", function(directiveService) {
  return {
    restrict: "E",
    scope: directiveService.autoScope({
      ngParameters: '='
